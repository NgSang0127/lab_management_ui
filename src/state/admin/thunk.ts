...
import {CreateUserRequestByAdmin} from "./adminSlice.ts";
import {PageResponse} from "../page/types.ts";
...
export const transferOwnership = createAsyncThunk(
    ...
    async (newOwnerId: number, { rejectWithValue }) => {
        try {
            ...
            return response.data.message || 'Ownership transferred successfully';
        } catch (error: unknown) {
            if (axios.isAxiosError(error)) {
                if (error.response && error.response.data) {
                    const backendError = error.response.data.error || error.response.data.message || 'Unknown backend error';
...
export const promoteUser = createAsyncThunk(
    ...
    async (userId: number, { rejectWithValue }) => {
        try {
            ...
            return response.data.message || 'user promoted successfully';
        } catch (error: unknown) {
            if (axios.isAxiosError(error)) {
                if (error.response && error.response.data) {
                    const backendError = error.response.data.error || error.response.data.message || 'Unknown backend error';
